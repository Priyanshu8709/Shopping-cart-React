...
const Product = ({ post }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-between hover:scale-105 transition duration-300 ease-in gap-3 p-4 mt-8 rounded-lg shadow-lg hover:shadow-2xl border border-gray-200 w-full md:max-w-[280px]">
      <div className="w-full">
        <p className="text-gray-800 font-semibold text-base text-left truncate mt-1">{title}</p>
      </div>
      <div className="w-full">
        <p className="text-gray-500 font-normal text-xs text-left">{post.description.split(" ").slice(0,10).join(" ")+"..."}</p>
      </div>
      <div>
      ...
      <div className="w-full">
        {image ? (
          <div className="h-[160px] md:h-[160px] flex items-center justify-center rounded-lg bg-white w-full">
